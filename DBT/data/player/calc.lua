...
function player.calc_bonuses(silent)
	...
	tinsert(lines, "#yNOTE: Equipment modifiers are not displayed here!")
	tinsert(ac_lines, "#gArmor Class")
	tinsert(speed_lines, "#gSpeed")
	tinsert(combat_lines, "#gLeft Hand")
	tinsert(combat_lines, "#gRight Hand")
	tinsert(misc_lines, "#gMisc")
	...
	if has_ability(AB_SUSHI) then
		...
		tinsert(misc_lines, " * Sushi Eating Ability allows you to eat in zero time")
	end
	...
	if player.inventory[INVEN_HEAD][1] then
		if player.inventory[INVEN_HEAD][1].flags[FLAG_BE_BLIND] then
			...
			tinsert(misc_lines, " * Wearing a blindfold renders you blind")
		end
	end
	...
	if dball_data.p_xpmod > 0 then
		tinsert(misc_lines, " * You receive an XP bonus from equipment: " .. dball_data.p_xpmod)
	end
	...
	if dball_data.alive ~= 0 then
		...
		tinsert(misc_lines, "You are dead")

		-- Ghosts don't need to eat
		player.set_intrinsic(FLAG_FOOD_VALUE, 0)
		tinsert(misc_lines, " * You don't need to eat")

		-- Speed is based on intelligence
		player.pspeed = player.pspeed + player.stat(A_INT) - 10
		tinsert(speed_lines, " Being dead, your base speed is Intelligence - 10: " .. player.pspeed)
		...
		if dball_data.chi_haste_setting ~= 0 then
			...
			tinsert(speed_lines, " * Chi Haste increases your speed by: " .. dball_data.chi_haste_setting)
		end

		-- Dodge, but no Dex bonus to AC
		if get_skill(SKILL_DODGE) ~= 0 then
			player.to_a = player.to_a + get_skill(SKILL_DODGE) + get_skill(SKILL_DODGE)
			tinsert(ac_lines, " * Dodge Skill increases your AC by: " .. get_skill(SKILL_DODGE) + get_skill(SKILL_DODGE))
			tinsert(ac_lines, " * No AC modifiers from Dexterity while dead")
		end
		...
		if player.inventory[INVEN_VEHICLE][1] then
			if player.inventory[INVEN_VEHICLE][1].flags[FLAG_VEHICLE_SET_SPEED] then
				...
				tinsert(speed_lines, " * You are riding a vehicle that sets your base speed at: " .. player.inventory[INVEN_VEHICLE][1].flags[FLAG_VEHICLE_SET_SPEED])
			else
				tinsert(speed_lines, "You're riding a vehicle that doesn't set speed. REPORT THIS AS A BUG!")
			end
		else
			-- Default speed settings
			-- Apply speed stat to general speed
			player.pspeed = player.pspeed + player.stat(A_SPD) - 10
			tinsert(speed_lines, " * Your base speed is your net speed attribute - 10: " .. player.stat(A_SPD) - 10)

			-- Apply "encumbrance" from weight
			if encumberance ~= 0 then
				player.pspeed = player.pspeed - encumberance
				tinsert(speed_lines, " * The weight you are carrying slows you by: " .. encumberance)
			end
		...
		if dball.current_location() == "Kaio" then
			...
			tinsert(speed_lines, " * Gravity on the North Kaio's World slows you by: 30")
		end
	      -- Gravity on Namek is high, too
		if dball.am_i_on_namek() == true then
			player.pspeed = player.pspeed - 15
			tinsert(speed_lines, " * Gravity on Namek slows you by: 15")
		end
		...
		-- tinsert(speed_lines, " * Your net speed should be: " .. player.pspeed)

		tinsert(lines, "")
		tinsert(lines, "#gArmor Class")
		...
		if not player.inventory[INVEN_VEHICLE][1] then
			...
			tinsert(ac_lines, " * Your dexterity modifies your AC by net attribute -10, for net of: " .. player.stat(A_DEX) - 10)
			
			if get_skill(SKILL_DODGE) ~= 0 then
				player.to_a = player.to_a + get_skill(SKILL_DODGE) + get_skill(SKILL_DODGE)
				tinsert(ac_lines, " * Dodge skill increases your AC by 2 per skill, for a total of: " .. get_skill(SKILL_DODGE))
			end
		else
			-- While riding, Dex PENALTIES do apply, but no dex bonuses, and no Dodge bonuses
			if player.stat(A_DEX) < 10 then
				player.to_a = player.to_a + player.stat(A_DEX) - 10
				tinsert(lines, " * While riding a vehicle, dexterity PENALTIES do apply to AC: " .. player.stat(A_DEX) - 10)
			else
				tinsert(lines, " * You do not receive Dex bonuses to AC while riding vehicles")
			end
			if get_skill(SKILL_DODGE) ~= 0 then
				tinsert(lines, " * You do not receive Dodge bonuses to AC while riding vehicles")
			end
...
--[[
	...
	tinsert(lines, "")
	tinsert(lines, "#gBase to-hit values for barehand fighting")

	if player.stat(A_DEX) ~= 10 then
		tinsert(lines, " * Your base barehand-to-hit is your Dexterity(" .. player.stat(A_DEX) .. ") minus 10: " .. player.stat(A_DEX) - 10)
		m_hand_th = player.stat(A_DEX) - 10
	end
	if get_skill(SKILL_MARTIALARTS) ~= 0 then
		tinsert(lines, " * Plus your MartialArts skill: " .. get_skill(SKILL_MARTIALARTS))
		m_hand_th = m_hand_th + get_skill(SKILL_MARTIALARTS)
	end
	if get_skill(SKILL_HAND) ~= 0 then
		tinsert(lines, " * Plus your Barehand skill: " .. get_skill(SKILL_HAND))
		m_hand_th = m_hand_th + get_skill(SKILL_HAND)
	end

	----------------------------------------

	tinsert(lines, "")
	tinsert(lines, "#gBase to-hit values for weapon fighting")

	if player.stat(A_DEX) ~= 10 then
		tinsert(lines, " * Your base weapon to-hit is your Dexterity(" .. player.stat(A_DEX) .. ") minus 10: " .. player.stat(A_DEX) - 10)
		m_weap_th = player.stat(A_DEX) - 10
	end
	if get_skill(SKILL_MARTIALARTS) ~= 0 then
		tinsert(lines, " * Plus your MartialArts skill: " .. get_skill(SKILL_MARTIALARTS))
		m_weap_th = m_weap_th + get_skill(SKILL_MARTIALARTS)
	end
	if get_skill(SKILL_WEAPONS) ~= 0 then
		tinsert(lines, " * Plus your Weapons skill: " .. get_skill(SKILL_WEAPONS))
		m_weap_th = m_weap_th + get_skill(SKILL_WEAPONS)
	end

	----------------------------------------

	tinsert(lines, "")
	tinsert(lines, "#gBase melee damage for both barehand and weapon fighting")

	m_td = player.stat(A_STR) - 10

	if player.stat(A_STR) == 10 then
		tinsert(lines, " * You receive no melee combat bonus from strength")
	else
		tinsert(lines, " * Your base melee damage is your Strength(" .. player.stat(A_STR) .. ") minus 10: " .. m_td)
	end
	...
	if dball_data.chi_aura_setting == 1 then
		tinsert(lines, "")
		tinsert(lines, "Battle Aura")
		if get_skill(SKILL_CHI_OFFENSE) ~= 0 then
			tinsert(lines, " * Chi Offense adds to barehand to-hit: " .. get_skill(SKILL_CHI_OFFENSE))
			tinsert(lines, " * Chi Offense adds to weapon to-hit: " .. get_skill(SKILL_CHI_OFFENSE))
			tinsert(lines, " * Chi Offense adds to all melee damage: " .. get_skill(SKILL_CHI_OFFENSE))
		end
	...
	if player.inventory[INVEN_VEHICLE][1] then
		if has_flag(player.inventory[INVEN_VEHICLE][1], FLAG_KINTO_UN) then
			...
			tinsert(lines, " * You are riding Kinto Un, but are still able to fight")
		elseif has_flag(player.inventory[INVEN_VEHICLE][1], FLAG_TECHNO_COMBAT) then
			tinsert(lines, " * (IMPLEMENT ME!) You are wearing a Technomancer Combat Suit")
		else
			tinsert(lines, " * You are riding a vehicle that prevents you from ordinary melee combat")
			if w1 == 0 then
				w1 = -1
			end
			w2 = -1
	...
	tinsert(lines, "")
	tinsert(lines, "#gCombat")

	-- Two non weapons...
	if w1 == -1 and w2 == -1 then
		-- Do nothing!
		tinsert(lines, " * Neither hand is available for combat")
	...
	elseif w1 == 0 and w2 == 0 then
		...
		if has_ability(AB_DOUBLE_ATTACK) then
			...
			tinsert(lines, " * You are fighting barehand with both hands")
			if has_ability(AB_BLOCKING) then
				player.to_a = player.to_a + get_skill(SKILL_HAND) + get_skill(SKILL_HAND)
				tinsert(lines, " * Blocking increases your AC by your barehand skill, for each hand, net: " .. get_skill(SKILL_HAND) + get_skill(SKILL_HAND))
			end
		else
			dball_data.lhand_th = m_hand_th
			dball_data.lhand_td = m_td
			dball_data.rhand_valid = -1 -- Force disable second hand
			tinsert(lines, " * You only have the ability to fight effectively with one hand")
			if has_ability(AB_BLOCKING) then
				player.to_a = player.to_a + get_skill(SKILL_HAND)
				tinsert(lines, " * Blocking increases your AC by your barehand skill: " .. get_skill(SKILL_HAND))
			end
		end
	...
	elseif (w1 == 1 and w2 == 0) then
		-- First, compute for the weapon
		local w_diff = player.inventory[INVEN_WIELD][1].flags[FLAG_DIFFICULTY]
		dball_data.lhand_th = 0 - w_diff + m_weap_th 
		tinsert(lines, " * Your weapon difficulty reduces your to-hit chance: -" .. w_diff)
		dball_data.lhand_td = m_td
		player.to_a = player.to_a + get_skill(SKILL_PARRYING)
		-- Do we have ultimate fighting?
		if has_ability(AB_ULTIMATE) then
			tinsert(lines, " * Ultimate Fighting ability allows you to fight weapons+barehand together")
			-- Yes! Compute for Barehand
			dball_data.rhand_th = m_hand_th
			dball_data.rhand_td = m_td
			-- Add our AC bonus from Barehand:
			player.to_a = player.to_a + get_skill(SKILL_HAND)
			tinsert(lines, " * Ultimate Fighting ability allows you to receive barehanded AC bonus from one hand: " .. get_skill(SKILL_HAND))
		else
			-- No, disable the hand
			dball_data.rhand_valid = -1
			-- But, if we have double attack, then we at least get to defend with it.
			if has_ability(AB_DOUBLE_ATTACK) then
				if has_ability(AB_BLOCKING) then
					player.to_a = player.to_a + get_skill(SKILL_HAND)
					tinsert(lines, " * Blocking ability allows you to receive barehanded AC bonus from your empty hand: " .. get_skill(SKILL_HAND))
				end
			end
		end
	...
	elseif (w1 == 1 and w2 == -1) then
		...
		tinsert(lines, " * Your weapon difficulty reduces your to-hit chance: -" .. w_diff)
		dball_data.lhand_td = m_td
		if get_skill(SKILL_PARRYING) > 0 then
			player.to_a = player.to_a + get_skill(SKILL_PARRYING)
			tinsert(lines, " * Your Parrying skill adds to your AC " .. get_skill(SKILL_PARRYING))
		end
	...
	if player.inventory[INVEN_VEHICLE][1] then
		if dball_data.lhand_valid > 0 then
			...
			tinsert(lines, " * Melee to-hit is penalized while riding a motorcycle: 30")
		end
	end
