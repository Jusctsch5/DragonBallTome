...
load_subsystem("curr_know",
{
	...
	sort_uniques_by_level =
		function(list)
			-- Sort list by monster level
			-- or by name if the levels are the same
			for i = 1, getn(list) do
				local level = list[i].monst.level

				-- Do we want to do this for the end game uniques?
...
add_regexp_plural {
	...
	-- Plurals that are the same as the singular
	["fish$"] = "fish",
	["ninja$"] = "ninja",
}
...
add_regexp_plural("^(creeping .* coins)$", "piles of %1")
