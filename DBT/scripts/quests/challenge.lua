...
add_quest
{
	...
	desc =
	{
		"One of the dancers at the local Ballet studio has gone insane, and",
		"disappeared with eight children. Can you find them and bring them back,",
		"alive?",
	}
	level = 15
	hooks =
...
add_quest
{
	global = "QUEST_CHALLENGE_BALLET"
	name = "Destroy Sakura's Troupe"
	desc =
	{
		"Sakura's Dance school is a cleverly disguised mech workshop. Grandmaster",
		"Hatsumi has asked you to infiltrate the school and destroy the mechs.",
	}
	level = 30
	hooks =
	{
...
-- NOTE: As of DBT V8 or so, this quest no longer exists, and neither does Elizabeth
add_quest
{
...
add_quest
{
	...
	desc = function()
		if quest(QUEST_DOJO_DESTROYER).status == QUEST_STATUS_TAKEN then
			local line = {
				"#yDojo Destroyer (Danger level: 23)",
				"The fewer martial arts schools there are in the area, the larger the remaining schools will become and thus, the better training they will be able to offer. More schools means better variety, but fewer schools means higher skill. Which schools will you train at? Which schools will you destroy?",
			...
			if enrollments[FLAG_SUMO] > 0 then
				if quest(QUEST_CHALLENGE_JUDO).status == QUEST_STATUS_FINISHED then
					...
					tinsert(line, " * Sumo (Must defeat Judo to qualify)")
				end
			end
			if enrollments[FLAG_JUDO] > 0 then
				if quest(QUEST_CHALLENGE_SUMO).status == QUEST_STATUS_FINISHED then
					...
					tinsert(line, " * Judo (Must defeat Sumo to qualify)")
				end
			end
			if enrollments[FLAG_BALLET] > 0 then
				if quest(QUEST_CHALLENGE_NINJUTSU).status == QUEST_STATUS_FINISHED then
					...
					tinsert(line, " * Ballet (Must defeat Ninjutsu to qualify)")
				end
			end
			if enrollments[FLAG_NINJUTSU] > 0 then
				if quest(QUEST_CHALLENGE_BALLET).status == QUEST_STATUS_FINISHED then
					tinsert(line, " * Ninjutsu")
				else
					tinsert(line, " * Ninjustu (Must defeat Ballet to qualify)")
				end
			end
			tinsert(line, "Your training caps at these schools will increase by destroying:")
			if enrollments[FLAG_KARATE] == 0 then tinsert(line, " * Karate") end
			if enrollments[FLAG_KICKBOXING] == 0 then tinsert(line, " * Kickboxing") end
			if enrollments[FLAG_KUNGFU] == 0 then tinsert(line, " * Kung fu") end
			if enrollments[FLAG_FENCING] == 0 then tinsert(line, " * Fencing") end
			if enrollments[FLAG_SUMO] == 0 then tinsert(line, " * Sumo") end
			if enrollments[FLAG_JUDO] == 0 then tinsert(line, " * Judo") end
			if enrollments[FLAG_BALLET] == 0 then tinsert(line," * Ballet") end
			if enrollments[FLAG_NINJUTSU] == 0 then tinsert(line, " * Ninjutsu") end
			return line
		end
	end
	level = 23
