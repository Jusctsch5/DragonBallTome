...
-- On an item to indicate which TV/SV ranges will be randomly needed to build the item
-- On an ego to indicate which kidx will be required to build the item
...
-- On an item to indicate that it may be ego-enhanced
-- On an ego to indicfate that it may be ego-enhanced
new_flag("TECHNO_ENHANCEABLE")

-- For ego types which require NO
-- ingredients to make
new_flag("TECHNO_EGO_FREE")
...
function technomancy.dismantle(obj, kidx)
	...
	-- This will need to change if we allow multiple ego items
	if obj_for_ego.ego_id[1] and obj_for_ego.ego_id[1] ~= 0 then
		if tech_ego[obj_for_ego.ego_id[1]] ~= true and (adj_diff >= rng(100)) then
			...
			ego = e_info[1 + obj_for_ego.ego_id[1]]
			ego_message = "You think you could enhance a new one to be " .. ego.name .. " too!"
		end
	end
	if obj_for_ego.ego_id[2] and obj_for_ego.ego_id[2] ~= 0 then
		if tech_ego[obj_for_ego.ego_id[2]] ~= true and (adj_diff >= rng(100)) then
			...
			-- ego_message = "You think you could enhance things in a similar way"
			ego = e_info[1 + obj_for_ego.ego_id[2]]
			ego_message = "You think you could enhance a new one to be " .. ego.name .. " too!"
		end
	end
...
-- May this item acquire this ego?
function technomancy.ego_ok(tv, sv, ego)
	if e_info[ego + 1].item_kinds_ok[tv][sv] == true then
		return true
	else
		return false
	end
		...
		if zoom_mode == 0 then
			...
			for i = 1, getn(recipe_list) do
				...
				if has_flag(item, FLAG_DISCIPLINE) and item.flags[FLAG_DISCIPLINE] == discipline then
					...
					term.blank_print(color.WHITE, strchar(d_count + 96) .. ") " .. item.name, d_count + d_vertmod + 1, d_horiz)
			...
			local show_egos = false
			local ego_hack = "You know how to add the following enhancements:\n"
			for ego_count = 2, max_e_idx do
				if technomancy.ego_ok(obj.tval, obj.sval, ego_count) then
					if tech_ego[ego_count] == true then
						show_egos = true
						ego_hack = ego_hack .. " * " .. e_info[1 + ego_count].name .. "\n"
					end
				end
			end
			...
			-- Display known enhanceable ego types
			if show_egos == true then
				term.text_out(ego_hack)
			end
...
function technomancy.interface_to_enhance(discipline)
	-- Get an item
	local ret, item = get_item("Attempt to enhance which item?",
							   "You don't have anything that can be enhanced",
							   USE_INVEN | USE_EQUIP, -- allow enhancment of equipped items?
							   function(o)
...
function technomancy.discipline_name(which)
	if which == 1 then
		return "a mechanical toolkit"
	elseif which == 2 then
		return "an electrical toolit"
	elseif which == 2 then
		return "a chemistry kit"
	elseif which == 4 then
		return "a lathe"
	else
