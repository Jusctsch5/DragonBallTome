function dball.determine_class()
...
function dball.calc(silent, disp)
	if not disp then
		disp = false
	end
	...
	-- What class are we?
	dball_data.p_class = dball.determine_class()
	...
	if dball_data.alive ~= 0 then
		...
		if dball_data.chi_haste_setting ~= 0 then
			...
			tinsert(lines, " * Chi Haste increases your speed by: " .. dball_data.chi_haste_setting)
		end
		...
		if get_skill(SKILL_DODGE) ~= 0 then
			...
			tinsert(lines, " * Dodge Skill increases your AC by: " .. get_skill(SKILL_DODGE) + get_skill(SKILL_DODGE))
			tinsert(lines, " * No AC modifiers from Dexterity while dead")
		end
		...
		if player.inventory[INVEN_VEHICLE][1] then
			if player.inventory[INVEN_VEHICLE][1].flags[FLAG_VEHICLE_SET_SPEED] then
				...
				tinsert(lines, " * You are riding a vehicle that sets your base speed at: " .. player.inventory[INVEN_VEHICLE][1].flags[FLAG_VEHICLE_SET_SPEED])
			else
				tinsert(lines, "You're riding a vehicle that doesn't set speed. REPORT THIS AS A BUG!")
			end
		else
			-- Default speed settings
			-- Apply speed stat to general speed
			player.pspeed = player.pspeed + player.stat(A_SPD) - 10
			tinsert(lines, " * Your base speed is your net speed attribute - 10: " .. player.stat(A_SPD) - 10)

			-- Apply "encumbrance" from weight
			if encumberance ~= 0 then
				player.pspeed = player.pspeed - encumberance
				tinsert(lines, " * The weight you are carrying slows you by: " .. encumberance)
			end
		...
		if dball.current_location() == "Kaio" then
			...
			tinsert(lines, " * Gravity on the North Kaio's World slows you by: 30")
		end
	      -- Gravity on Namek is high, too
		if dball.am_i_on_namek() == true then
			player.pspeed = player.pspeed - 15
			tinsert(lines, " * Gravity on Namek slows you by: 15")
		end
		...
		-- tinsert(lines, " * Your net speed should be: " .. player.pspeed)

		tinsert(lines, "Armor Class")
		...
		if not player.inventory[INVEN_VEHICLE][1] then
			...
			tinsert(lines, " * Your dexterity modifies your AC by net attribute -10, for net of: " .. player.stat(A_DEX) - 10)
			
			if get_skill(SKILL_DODGE) ~= 0 then
				player.to_a = player.to_a + get_skill(SKILL_DODGE) + get_skill(SKILL_DODGE)
				tinsert(lines, " * Dodge skill increases your AC by 2 per skill, for a total of: " .. get_skill(SKILL_DODGE))
			end
		else
...
--[[
	...
	tinsert(lines, "")
	tinsert(lines, "Combat: Base to-hit values for barehand fighting")

	if player.stat(A_DEX) ~= 10 then
		tinsert(lines, " * Your dexterity modifies your barehand to-hit by Dex -10, for a net of: " .. player.stat(A_DEX) - 10)
		m_hand_th = player.stat(A_DEX) - 10
	end
	if get_skill(SKILL_MARTIALARTS) ~= 0 then
		tinsert(lines, " * Martial Arts skill modifies your melee to-hit: " .. get_skill(SKILL_MARTIALARTS))
		m_hand_th = m_hand_th + get_skill(SKILL_MARTIALARTS)
	end
	if get_skill(SKILL_HAND) ~= 0 then
		tinsert(lines, " * Barehand skill increases your barehanded to-hit: " .. get_skill(SKILL_HAND))
	...
	tinsert(lines, "")
	tinsert(lines, "Combat: Base to-hit values for weapon fighting")

	if player.stat(A_DEX) ~= 10 then
		tinsert(lines, " * Your dexterity modifies your weapon to-hit by Dex -10, for a net of: " .. player.stat(A_DEX) - 10)
		m_weap_th = player.stat(A_DEX) - 10
	end
	if get_skill(SKILL_MARTIALARTS) ~= 0 then
		tinsert(lines, " * Martial Arts skill modifies your weapon to-hit: " .. get_skill(SKILL_MARTIALARTS))
		m_weap_th = m_hand_th + get_skill(SKILL_MARTIALARTS)
	end
	if get_skill(SKILL_WEAPONS) ~= 0 then
		tinsert(lines, " * Barehand skill increases your weapon to-hit: " .. get_skill(SKILL_WEAPONS))
		m_weap_th = m_hand_th + get_skill(SKILL_WEAPONS)
	end

	----------------------------------------

	tinsert(lines, "")
	tinsert(lines, "Combat: Base melee damage for both barehand and weapon fighting")
	...
	if player.stat(A_STR) == 10 then
		...
		tinsert(lines, " * Your base melee damage is your Strength -10, for a net of: " .. m_td)
	end
...
-- ******************* End Equipment Bonuses ************************
	...
	-- If we've been invoked by the Knowlegde menu
	-- then return the full calc description
	if disp == true then
		return lines
	end
